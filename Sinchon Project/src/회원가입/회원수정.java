...
public class 회원수정 {
	...
	public void modify() {
		...
		btnmodify.setBounds(269, 574, 97, 40);
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					MemberVO bag = dao.all(id);
					UserPW.setText(bag.getUserPW());
					UserName.setText(bag.getUserName());
					UserBirth.setText(bag.getUserBirth());
					UserTel.setText(bag.getUserTel());
					gender = bag.getUserGender();

					if (gender.equals("M")) {
						mradiobtn.setSelected(true);
					} else {
						fmradiobtn.setSelected(true);
					}
				} catch (Exception e1) {
					// TODO Auto-generated catch block
					JOptionPane.showMessageDialog(j, "검색결과가없습니다");
					e1.printStackTrace();
				}

			}
		});
		btnNewButton.setBounds(431, 132, 108, 23);
		...
		btnmodify.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				} else if (id.isEmpty() || name.isEmpty() || birth.isEmpty() || tel.isEmpty() || gender.isEmpty()
						|| pw.isEmpty()) {
					JOptionPane.showMessageDialog(j, "비어있는 칸이 존재합니다.");
				}

				else {
					JOptionPane.showMessageDialog(j, "수정을 완료했습니다.");

					try {
						db.update(id, pw, name, birth, tel, gender);
						j.dispose();
					} catch (Exception e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
